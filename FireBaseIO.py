...
class FireBase:
    ...
    def GetD(self):
        # db = firestore.client()
        self.users_ref = db.collection(u'DummyMLAIHardik')
        self.docs = self.users_ref.stream()
        for doc in self.docs:
            self.user_data["Drinking"] = u'{}'.format(doc.to_dict()['d'])
            self.user_data["Smoking"] = u'{}'.format(doc.to_dict()['s'])
            self.user_data["Gender"] = u'{}'.format(doc.to_dict()['g'])
            # self.user_data["Name"] = u'{}'.format(doc.to_dict()['n'])
            self.user_data["Id"] = u'{}'.format(doc.id)
        return(self.user_data)

    def SendD(self, cluster, id):
        self.users_refx = db.collection(u'DummyMLAIHardi').document(id).collection(u'OnBdCluster')
